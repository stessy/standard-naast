...
public class GenericDAOImpl {
	...
	public <T> T persist(final T entity) {
		...
		// GenericDAOImpl.LOG.debug("Persisting the entity [" +
		// entity.toString()
		// + "]");
	...
	public <T> List<T> findAll(final Class<T> entityType) {
		...
		// + entityType.toString() + "]");
		final CriteriaQuery<T> queryAll = this.getEntityManager()
	...
	public void remove(final Object entity) {
		...
		// + "]");
		final Object mergedEntity = this.getEntityManager().merge(entity); // merge
																			// it
																			// just
																			// to
																			// be
	...
	public <T> T refresh(final T entity) {
		// GenericDAOImpl.LOG.debug("Refreshing the entity [" +
		// entity.toString()
		// + "]");
		final Object mergedEntity = this.getEntityManager().merge(entity); // merge
																			// it
	...
	public <T> List<T> refresh(final List<T> entities) {
		final List<T> refreshedEntities = new ArrayList<T>();
		for (final T entity : entities) {
	...
	public <T> T getReference(final Class<T> entityClass,
			final Object primaryKey) {
		// GenericDAOImpl.LOG.debug("Getting reference for entity [" +
		// primaryKey
		// + "]");
	...
	public <T> List<T> getNamedQueryResultList(final String name,
		...
		final TypedQuery<T> typedQuery = this.getEntityManager()
	...
	public <T> List<T> getCriteriaBuilderResultList(
		...
		final CriteriaQuery<T> cQuery = cqb.createCriteriaQuery(cb);
		final TypedQuery<T> tQuery = this.getEntityManager()
				.createQuery(cQuery);
	...
	private void setQueryParameters(final Query query,
		...
		for (int position = 0; position < parameters.length; position++) {
			final Object parameter = parameters[position];
	...
	private void validateQuery(final Query query) {
		...
		for (final Parameter<?> param : parameters) {
			final Matcher paramMatcher = GenericDAOImpl.VALID_PARAM
					.matcher(param.getName());
