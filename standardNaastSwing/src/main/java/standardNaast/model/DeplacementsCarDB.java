...
public class DeplacementsCarDB {
    ...
    public Vector<Vector<Vector<Object>>> getDeplacementsCar() throws Exception {
        ...
        try {
            ...
            for (; rs.next(); allDeplacementsCar.add(deplacementCar)) {
                ...
                deplacementCar.add(rs.getInt(7) != 0
    ...
    public boolean addDeplacementCar(Vector<?> data) throws Exception {
        ...
        try {
            if (deplacementExistsOnAdd(data)) {
                ...
                insertStatement =
                        connection.prepareStatement("insert into prix_locomotion (PRIX_LOCOMOTION_ID,ANNEE,MONTANT,LIEU,MEMBRE,AGE_MINIMUM,AGE_MAXIMUM)\nvalues(prix_locomotion_seq.nextval,?,?,?,?,?,?)");
    ...
    public boolean updateDeplacementCar(Vector<?> data) throws Exception {
        ...
        try {
            ...
            if (deplacementExistsOnUpdate(data)) {
                ...
                connection = JDBCConnection.getInstance().getJDBCConnection();
                updateStatement =
                        connection.prepareStatement("update prix_locomotion set annee = ?,montant = ?,lieu = ?,membre = ?,age_minimum = ?,age_maximum = ? where prix_locomotion_id = ?");
    ...
    public int[] getPrixDeplacementParDateNaissance(int personneID,
            int matchID) throws Exception {
        Connection connection;
        PreparedStatement selectStatement = null;
        int membre;
        int info[];
        ...
        info = new int[2];
        try {
            System.out.println((new StringBuilder()).append("Personne ID: ").append(personneID).toString());
            System.out.println((new StringBuilder()).append("Match ID: ").append(matchID).toString());

            selectStatement =
                    connection.prepareStatement("select to_char(sysdate,'YYYY') from dual");
            rs = selectStatement.executeQuery();
            rs.next();
            annee = rs.getString(1);
            rs.close();
            selectStatement.close();
            System.out.println((new StringBuilder()).append("Ann�e: ").append(annee).toString());
            selectStatement =
                    connection.prepareStatement("select count(*) from personnes_cotisations where personne_id = ? and annee_cotisation = to_number(to_char(sysdate,'YYYY'))");
            ...
            if (membre == 0) {
                selectStatement =
                        connection.prepareStatement("select sysdate from dual where to_date(?,'DD-MM-YYYY') > sysdate");
                selectStatement.setString(1,
                        (new StringBuilder()).append("31-03-").append(annee).toString());
                rs = selectStatement.executeQuery();
                if (rs.next()) {
                    PreparedStatement selectStatement2 =
                            connection.prepareStatement("select count(*) from personnes_cotisations where personne_id = ? and annee_cotisation = to_number(to_char(sysdate,'YYYY')) - 1");
            ...
            System.out.println((new StringBuilder()).append("Membre: ").append(membre).toString());
            selectStatement =
                    connection.prepareStatement("select to_char(date_naissance,'DD-MM-YYYY')  from personnes where personne_id = ?");
            selectStatement.setInt(1, personneID);
            rs = selectStatement.executeQuery();
            rs.next();
            ...
            rs.close();
            selectStatement.close();
            System.out.println((new StringBuilder()).append("Date de naissance: ").append(dateNaissance).toString());

            selectStatement =
                    connection.prepareStatement("select to_char(date_match,'DD-MM-YYYY') from match where match_id = ?");
            ...
            rs.close();
            selectStatement.close();
            System.out.println((new StringBuilder()).append("Date de match: ").append(dateMatch).toString());
            selectStatement =
                    connection.prepareStatement((new StringBuilder()).append("select trunc(((((86400*(to_date('").append(dateMatch).append("','DD-MM-YYYY') - to_date('").append(dateNaissance).append("','DD-MM-YYYY')))/60)/60)/24)/365) from dual").toString());
            rs = selectStatement.executeQuery();
            rs.next();
            anneesDifference = rs.getInt(1);
            rs.close();
            selectStatement.close();
            System.out.println((new StringBuilder()).append("Ann�es diff�rence: ").append(anneesDifference).toString());
            selectStatement =
                    connection.prepareStatement("select lieu from match where match_id = ?");
            selectStatement.setInt(1, matchID);
            rs = selectStatement.executeQuery();
            rs.next();
            lieu = rs.getString(1);
            rs.close();
            selectStatement.close();
            System.out.println((new StringBuilder()).append("Lieu: ").append(lieu).toString());
            selectStatement =
                    connection.prepareStatement("select montant, prix_locomotion_id from prix_locomotion where annee = to_number(to_char(sysdate,'YYYY')) and lieu = ? and membre = ? and age_minimum <= ? and age_maximum > ?");
            ...
            rs.close();
            selectStatement.close();
            System.out.println((new StringBuilder()).append("Montant: ").append(info[0]).toString());
            System.out.println((new StringBuilder()).append("Prix locomotion ID: ").append(info[1]).toString());
            return info;
        } catch (Exception e) {
            e.printStackTrace();
            throw new Exception(e.getMessage());
    ...
    public void ajouterDeplacementCarMembre(int personneID, int matchID,
        ...
        try {
            connection = JDBCConnection.getInstance().getJDBCConnection();
            insertStatement =
                    connection.prepareStatement("insert into personnes_match (MATCH_ID,PERSONNE_ID,PRIX_LOCOMOTION_ID)\nvalues(?,?,?)");
    ...
    public void effacerDeplacementCarMembre(int personneID, int matchID,
        ...
        try {
            connection = JDBCConnection.getInstance().getJDBCConnection();
            deleteStatement =
                    connection.prepareStatement("delete from personnes_match where match_id = ? and personne_id = ? and prix_locomotion_id = ?");
    ...
    public int calculerTotalDeplacementcar(int matchID) throws Exception {
        ...
        try {
            connection = JDBCConnection.getInstance().getJDBCConnection();
            selectStatement =
                    connection.prepareStatement("select sum(montant) from prix_locomotion pl,personnes_match pm where pl.PRIX_LOCOMOTION_ID = pm.PRIX_LOCOMOTION_ID\nand match_id = ?");
            selectStatement.setInt(1, matchID);
            rs = selectStatement.executeQuery();
            rs.next();
            total = rs.getInt(1);
    ...
    public int calculerTotalPersonnesDeplacementcar(int matchID) throws Exception {
        PreparedStatement selectStatement = null;
        int total;
        Connection connection = null;
        selectStatement = null;
        ...
        try {
            connection = JDBCConnection.getInstance().getJDBCConnection();
            selectStatement =
                    connection.prepareStatement("select count(*) from personnes_match pm where match_id = ?");
            selectStatement.setInt(1, matchID);
            rs = selectStatement.executeQuery();
            rs.next();
            total = rs.getInt(1);
    ...
    public static void main(String args[]) {
        ...
        try {
            int i[] = deplacementCarDB.getPrixDeplacementParDateNaissance(457, 2342);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
