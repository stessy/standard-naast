...
public class MatchDB {
	...
	public Vector getDeplacementsCarParMatch(final int matchID,
			final boolean isMembre) throws Exception {
		Connection connection;
		PreparedStatement selectStatement = null;
		...
		try {
			String string = (new StringBuilder())
					.append("select p.personne_id, nom || ' ' || prenom \"Nom et Prénom\", 'true' payé from personnes p inner join personnes_match pm on p.personne_id = pm.personne_id where match_id = ? and numero_membre ")
					.append(isMembre ? "<" : ">=")
					.append(" 10000 ")
					.append("union ")
					...
					.append(" on p.personne_id = pm.personne_id")
					.append(" where match_id = ?)").append(" order by 2")
					.toString();
			System.out.println(string);
			selectStatement = connection.prepareStatement(string);
			...
			for (; rs.next(); prixLocomotionID.add(new Integer(
					montantDeplacement[1]))) {
				deplacementsCar = new Vector();
				deplacementsCar.add(Integer.valueOf(rs.getInt(1)));
				deplacementsCar.add(rs.getString(2));
				montantDeplacement = deplacementCarDB
						.getPrixDeplacementParDateNaissance(rs.getInt(1),
								matchID);
				deplacementsCar.add(Integer.valueOf(montantDeplacement[0]));
				deplacementsCar.add(Boolean.valueOf(Boolean.parseBoolean(rs
						.getString(3))));
			...
			for (int i = 0; i < columnNames.size(); i++) {
				System.out.println((new StringBuilder()).append("ColumnName: ")
						.append(columnNames.get(i)).toString());
			}
	...
	public void ajouterMatch(final Vector data) throws Exception {
		...
		try {
			...
			connection.commit();
			JOptionPane.showMessageDialog(null, "Match ajoutï¿½",
					"Ajout match", 1);
		} catch (Exception e) {
			e.printStackTrace();
			throw new Exception(e.getMessage());
